const timeline = {
  ...
  init() {
    ...
    const sectionChildren = Array.from(this.slideList).filter(
      e => !e.id.includes("cover-")
    )
    const addTransitions = function(parent, toggle) {
      const parts = parent.querySelectorAll(".timeline-text, .timeline-image")
      if (toggle === 0) {
        // even; toggle allows for the image / text order switching on larger viewports
